...
symbol=["EURUSD", "USDJPY", "AUDUSD", "NZDUSD", "EURJPY", "GBPUSD", "GBPJPY", "NZDJPY", "AUDJPY"]
# symbol=["EURUSD", "USDJPY", "AUDUSD", "NZDUSD", "EURJPY", "GBPUSD"]
# symbol=["GBPUSD"]
...
def gen_data(symbol=symbol):
    ...
    for s in symbol:
        ...
        # r = mt5.copy_rates_from_pos(s, mt5.TIMEFRAME_H4, 0,20000)
        ...
        # df["sig"] = np.log(df.close / df.close.shift(1))
        #
        ...
        df["sig3"] = macd_sig
        df["sig4"] = np.log(df.close.ewm(span=10, adjust=False).mean() / df.close)
        # df["sig5"] = df.close.ewm(span=30, adjust=False).mean() - df.close.ewm(span=1, adjust=False).mean()
        # # df["sig6"] = df.close.ewm(span=5, adjust=False).mean() - df.close.ewm(span=1, adjust=False).mean()
        df["sig7"] = np.log(df.close.ewm(span=50, adjust=False).mean() / df.close)
